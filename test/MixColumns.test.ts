...
import { FieldList } from "../src/utils/list";
...
describe("GMixColumns", () => {
  ...
  it("generates correct key for single column input", async () => {
    ...
    expect(num.toArrayUnconstrained().get()).toEqual([
      Field(0x8e),
      Field(0x4d),
      Field(0xa1),
      Field(0xbc),
    ...
    expect(num2.toArrayUnconstrained().get()).toEqual([
      Field(0x9f),
      Field(0xdc),
      Field(0x58),
      Field(0x9d),
    ...
    expect(num3.toArrayUnconstrained().get()).toEqual([
      Field(0x01),
      Field(0x01),
      Field(0x01),
      Field(0x01),
    ...
    expect(num4.toArrayUnconstrained().get()).toEqual([
      Field(0xc6),
      Field(0xc6),
      Field(0xc6),
      Field(0xc6),
    ...
    expect(num5.toArrayUnconstrained().get()).toEqual([
      Field(0xd5),
      Field(0xd5),
      Field(0xd7),
      Field(0xd6),
    ...
    expect(num6.toArrayUnconstrained().get()).toEqual([
      Field(0x4d),
      Field(0x7e),
      Field(0xbd),
      Field(0xf8),
  ...
  it("Generates correct key for whole matrix inputs", async () => {
    ...
    expect(num.toField().toBigInt().toString(16)).toEqual(
      "8e9f01c64ddc01c6a15801c6bc9d01c6",
    );
    // input transposed
    // 0xdb, 0x13, 0x53, 0x45,
    // 0xf2, 0x0a, 0x22, 0x5c,
